...
(function () {
    ...
    function wemGoogleController($scope, _, jcrService, notificationService, loadingSpinnerService, i18nService, $http) {
        ...
        _.each(vm.persoObject.variants, function (variation) {
        ...
        function _saveExperiment() {
            ...
            if (vm.persoObjectNode.mixins.wemgooglemix__experiment) {
                ...
                request = gapi.client.analytics.management.experiments.update(vm.googleExperiment);
                ...
                request = gapi.client.analytics.management.experiments.insert(vm.googleExperiment);
                var nodesId = '';
                request.execute(function (response) {
                    if (response.code > 300) {
                        loadingSpinnerService.hide();
                        ...
                        jcrService.addMixin('default', null, vm.persoObject.nodeIdentifier, 'wemgooglemix:experiment', {
                            properties: {
                            ...
                            _.each(vm.persoObject.variants, function (variable, index) {
                                ...
                                jcrService.addMixin('default', null, variable.nodeIdentifier, 'wemgooglemix:variable', { properties: { wemgaVariableId: { value: vm.persoObjectNode.type == 'jnt:page' ? index + 1 : index }, wemgaVariableName: { value: variable.name.length > 250 ? variable.name.substring(0, 250) : variable.name } } }, false).then(function () {
                                    if (var_index == vm.persoObject.variants.length) {
                                        $http.post(ManagersContext.baseEdit + ManagersContext.currentSitePath + '.publishNodeAction.do?nodesid=' + nodesId, { nodesIdobject: nodesId }).then(function () {
                                            loadingSpinnerService.hide();
