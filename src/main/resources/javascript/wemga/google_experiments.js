...
function getGoogleFields(data){
    ...
    jahiaAPIStandardCall(jcrParameters.context, jcrParameters.workspace, jcrParameters.locale, "nodes", data.variant, "GET", {}, function(variantXhr){
        console.log("Get google fields 1");
        ...
        if(data.variant != data.variantWrapper){
            ...
            jahiaAPIStandardCall(jcrParameters.context, jcrParameters.workspace, jcrParameters.locale, "nodes", data.variantWrapper, "GET", {}, function(wrapperXhr){
                console.log("Get google fields 2");
...
function sendFeedToGoogle(googleFields){
    console.log("Sending feed to google analytoics 1 ");
    console.log(googleFields);
    if(googleFields.area != undefined && googleFields.area.experimentId !=undefined && googleFields.variant.variableId != undefined){
        console.log("Sending feed to google analytoics 2");
        //Create mv testing
        ...
        cxApi.setChosenVariation(googleFields.variant.variableId, googleFields.area.experimentId);
        console.log("Sending feed to google analytoics 4 ");
        console.log(googleFields);
        //Sending an event that will carry the mv/test
        ga('send', 'event', googleFields.type, 'load','UnomiData');
    } else {
        console.log("Sending feed to google analytoics 3 ");
        var ids = "";
        ...
        if(ids.length > 0){
            if(! googleFields.area.experimentId){
                console.warn("googleVariants.js - No experiment id defined, the page will be pushed without mv test");
            } else {
                console.warn("googleVariants.js - No variable id defined, the page will be pushed without mv test");
            }
        }
    }
}
console.log("1 Sending feed to google analytoics");
// Listen for the event.\n" +
document.addEventListener('displayWemVariant', function (e) {
    console.log("2 Sending feed to google analytoics");
    var data = e.detail;
    getGoogleFields(data);
}, true);
console.log("3 Sending feed to google analytoics");
